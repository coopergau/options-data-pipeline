...
This slide shows the price movement of a specific contract over time, compared to underlying asset price action as well as contract intrinsic value. This visual is particularly intersting becuase of how it illustrates theta decay. This call option had zero intrinsic value for almost the whole time period shown here, yet it was still trading around 50 cents per contract, with the price very slightly going down as time went on. However, on the last day shown here we can see that the price of the underlying asset moves above 590 so the contract gains intrinsic value and the price increases. We can also see how the price of the call and the put both converge towards their intrinsic value as their expiration date approaches.
